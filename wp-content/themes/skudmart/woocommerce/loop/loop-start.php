...
$active_shop_masonry = skudmart_get_option('active_shop_masonry', 'off');
$shop_masonry_column_type = skudmart_get_option('shop_masonry_column_type', 'default');
$woocommerce_shop_masonry_columns = skudmart_get_responsive_columns('woocommerce_shop_masonry_columns');

$product_masonry_container_width = skudmart_get_option('product_masonry_container_width', 1170);
$product_masonry_item_width = skudmart_get_option('product_masonry_item_width', 270);
$product_masonry_item_height = skudmart_get_option('product_masonry_item_height', 450);
$woocommerce_shop_masonry_custom_columns = skudmart_get_responsive_columns('woocommerce_shop_masonry_custom_columns');
$shop_masonry_item_setting = skudmart_get_option('shop_masonry_item_setting');


$woocommerce_pagination_type = skudmart_get_option('woocommerce_pagination_type', 'pagination');

$column_tmp = skudmart_get_responsive_columns('woocommerce_shop_page_columns');

if($is_main_loop && skudmart_string_to_bool($active_shop_masonry)){
...
$design = skudmart_get_option("shop_catalog_grid_style", '1');
...
if($is_main_loop && skudmart_string_to_bool($active_shop_masonry)){
    skudmart_set_wc_loop_prop('prods_masonry', true);
    ...
    if( $shop_masonry_column_type != 'custom' ){
        ...
        $loopCssClass[] = skudmart_get_responsive_column_classes('woocommerce_shop_masonry_columns', array(
            'mobile' => 1,
            'tablet' => 1
        ));
    }
        ...
        skudmart_set_wc_loop_prop('item_sizes', $__new_item_sizes);
        $masonry_component_type[] = 'AdvancedMasonry';
        $column_tmp = skudmart_get_responsive_columns('woocommerce_shop_masonry_custom_columns');
    }
    skudmart_set_wc_loop_prop('image_size', skudmart_get_option('product_masonry_image_size', 'shop_catalog'));
    ?>
<?php
}
else{
    $loopCssClass[] = 'grid-items';
    $loopCssClass[] = skudmart_get_responsive_column_classes('woocommerce_shop_page_columns');
}
...
if($is_main_loop){
    ...
    skudmart_set_wc_loop_prop('pagi_data', array(

...
<div id="la_compare_table_wrapper">
    ...
    $lists = Skudmart_WooCommerce_Compare::get_data();

    $default_attr = skudmart_get_wc_attribute_for_compare();
    $tax_attr = skudmart_get_wc_attribute_taxonomies();
    $all_attr = array_merge($default_attr, $tax_attr);

    $attribute_allow = skudmart_get_option('compare_attribute', $default_attr);
    ...
    if (!empty($lists)) {
        foreach ($lists as $product_id) {
            ...
            $term['id'] = $product_id;
            $term['sku'] = ($_product->get_sku() ? $_product->get_sku() : __('N/A', 'skudmart'));
        ...
        ?>
        <table class="la-compare-table la-compare-table-items">
            <tbody>
            <?php
            ...
            foreach( $attribute_allow as $attr_alow ){
                if(in_array($attr_alow, array('image','title','add-to-cart'))){
                    $label_list['info'] = esc_html_x('Product Info', 'front-view', 'skudmart');
                }
                else{
                ...
                foreach ($terms as $item) {
                    ...
                    elseif ($k == 'stock') {
                        ...
                        if ($item['availability'] && !empty($item['availability']['availability'])) {
                            ...
                            if ($item['stock']) {
                                $text_stock = __('In stock', 'skudmart');
                                $class_stock = 'in-stock';
                            } else {
                                $text_stock = __('Out of stock', 'skudmart');
        ...
        <table class="la-compare-table la-compare-table-heading">
            <tbody>
            <?php
            foreach ($label_list as $k => $v) {
                echo '<tr class="compare-tr compare-tr-'. esc_attr($k) .'">';
        ...
        ?>
        <p class="text-center"><?php esc_html_e('No products were added to the list', 'skudmart') ?></p>
        <?php
    }
    ?>
</div>

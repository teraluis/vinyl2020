...
<article class="lastudio-posts__item loop__item grid-item<?php if(has_post_thumbnail()) echo ' has-post-thumbnail'; ?>">
    <div class="lastudio-posts__inner-box"><?php
        ...
        if( $show_meta == 'yes' && $show_categories == 'yes') {
            skudmart_entry_meta_item_category_list('<div class="post-meta post-meta--top"><div class="post-terms post-meta__item">', '</div></div>', '');
        }
        ...
        if( $show_meta == 'yes' ) {
            ...
            if(filter_var($show_author, FILTER_VALIDATE_BOOLEAN)){
                echo sprintf(
                    ...
                    esc_html__( 'by ', 'skudmart' ),
                    esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
                    esc_html( get_the_author() ),
                    skudmart_get_schema_markup('author_name'),
                    skudmart_get_schema_markup('author_link')
                );
            }
            ...
            if(filter_var($show_date, FILTER_VALIDATE_BOOLEAN)){
                echo sprintf(
                    ...
                    skudmart_get_schema_markup('publish_date')
                );
            }

            if($show_comments == 'yes'){
                echo '<span class="post__comments post-meta__item"><i class="lastudioicon-b-meeting"></i>';
                comments_popup_link(__('0 comment', 'skudmart'),__('1 comment', 'skudmart'),false, 'post__comments-link');
                echo '</span>';
            }
        ...
        if($show_excerpt){
            ...
            echo sprintf(
                ...
                skudmart_excerpt(intval( $this->get_settings_for_display( 'excerpt_length' ) ))
            );
        }

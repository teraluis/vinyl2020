...
$role           = skudmart_get_post_meta(get_the_ID(), 'role');
...
$phone          = skudmart_get_post_meta(get_the_ID(), 'phone');
$email          = skudmart_get_post_meta(get_the_ID(), 'email');
        ...
        <div class="lastudio-team-member__inner">
            <div class="lastudio-team-member__image_wrap">
                ...
                if(in_array($preset, array('type-1', 'type-2', 'type-3'))){
                    echo '<div class="lastudio-team-member__cover"><div class="lastudio-team-member__socials">' . skudmart_get_member_social_tpl(get_the_ID()) . '</div></div>';
                }
                ?>
            </div>
            <div class="lastudio-team-member__content">
                ...
                if($excerpt_length > 0){
                    echo sprintf(
                        ...
                        skudmart_excerpt(intval( $excerpt_length ))
                    );
                }

                if(!in_array($preset, array('type-1', 'type-2', 'type-3'))){
                    echo '<div class="lastudio-team-member__socials">' . skudmart_get_member_social_tpl(get_the_ID()) . '</div>';
                }
                ?>
            </div>
        </div>

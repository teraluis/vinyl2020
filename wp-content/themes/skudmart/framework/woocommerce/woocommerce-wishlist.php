...
if(!class_exists('Skudmart_WooCommerce_Wishlist')){
    class Skudmart_WooCommerce_Wishlist {
        ...
        public function remove_item_from_url(){
            if(isset($_GET['la_helpers_wishlist_remove']) && ( $product_id = absint($_GET['la_helpers_wishlist_remove'])) ) {
                ...
                if(($key = array_search($product_id, $product_exists)) !== false) {
                    ...
                    wc_add_notice( sprintf( esc_html__('"%1$s" has been removed from wishlist', 'skudmart'), get_the_title($product_id) ) );
                }
            }
        }
        ...
        public function remove_from_wishlist_after_add_to_cart() {
            if( isset( $_REQUEST['la_remove_from_wishlist_after_add_to_cart'] ) ) {
                ...
                if(($key = array_search($product_id, $product_exists)) !== false) {
                    ...
                    $this->set_lists_for_cookie($product_exists);
                    self::$data = $product_exists;
                    wc_add_notice( sprintf( esc_html__('"%1$s" has been added to your cart.', 'skudmart'), get_the_title($product_id) ) );
                }
            }
        }
        ...
        public function ajax_process(){
            ...
            if(apply_filters('skudmart/check_ajax_referer', false, 'wishlist_nonce', 'security')){
            ...
            if( $check_ajax_referer ) {
                if(!isset($_REQUEST['post_id'])){
                    wp_send_json_error(array(
                        'message' => esc_html__('Invalid Product ID', 'skudmart')
                    ));
                }
                ...
                wp_send_json_error(array(
                    'message' => esc_html__('Invalid security key', 'skudmart')
                ));
            }
        }
        ...
        private function add( $post_id = 0 ){
            $lists = $this->get_data();
            $response = array();
            $response['wishlist_url'] = skudmart_get_wishlist_url();
            if(in_array($post_id, $lists)){
                $response['message'] = esc_html__('Product already in wishlist', 'skudmart');
                $response['has_error'] = true;
            }
            else{
                $response['message'] = esc_html__('Product added', 'skudmart');
        ...
        private function remove( $post_id = 0 ){
            ...
            if(($key = array_search($post_id, $lists)) !== false) {
                $response['message'] = esc_html__('Product has been removed from wishlist', 'skudmart');
                $response['has_error'] = false;
                unset($lists[$key]);
                $this->set_lists_for_user($lists);
                $this->set_lists_for_cookie($lists);
                ...
                $response['message'] = esc_html__('Product does not exist in wishlist', 'skudmart');
                $response['has_error'] = true;
            }
            $response['count'] = count( $lists );
            return $response;
...
new Skudmart_WooCommerce_Wishlist();
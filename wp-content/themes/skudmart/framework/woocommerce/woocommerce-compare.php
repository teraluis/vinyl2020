...
if(!class_exists('Skudmart_WooCommerce_Compare')){
    class Skudmart_WooCommerce_Compare {
        ...
        public function remove_item_from_url(){
            if(isset($_GET['la_helpers_compare_remove']) && ( $product_id = absint($_GET['la_helpers_compare_remove'])) ) {
                ...
                if(($key = array_search($product_id, $product_exists)) !== false) {
                    ...
                    wc_add_notice( sprintf( esc_html__('"%1$s" has been removed from list', 'skudmart'), get_the_title($product_id) ) );
                }
            }
        }
        ...
        public function ajax_process(){
            ...
            if(apply_filters('skudmart/check_ajax_referer', false, 'compare_nonce', 'security')){
        ...
        private function add( $post_id = 0 ){
            $lists = $this->get_data();
            $response = array();
            $response['compare_url'] = skudmart_get_compare_url();
            if(in_array($post_id, $lists)){
                $response['message'] = esc_html__('Product already in list', 'skudmart');
        ...
        private function remove( $post_id = 0 ){
            ...
            if(($key = array_search($post_id, $lists)) !== false) {
                $response['message'] = esc_html__('Product has been removed from list', 'skudmart');
                $response['has_error'] = false;
                unset($lists[$key]);
                $this->set_lists_for_user($lists);
                $this->set_lists_for_cookie($lists);
                ...
                $response['message'] = esc_html__('Product does not exist in list', 'skudmart');
                $response['has_error'] = true;
            }
...
new Skudmart_WooCommerce_Compare();
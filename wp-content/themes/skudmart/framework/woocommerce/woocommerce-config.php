...
if(!class_exists('Skudmart_WooCommerce_Config')){

    class Skudmart_WooCommerce_Config {
        ...
        public function __construct() {
            ...
            add_filter('skudmart/get_site_layout', array( $this, 'set_site_layout') );
            add_filter('skudmart/filter/sidebar_primary_name', array( $this, 'set_sidebar_for_shop'), 20 );
            ...
            if ( ! empty( $_REQUEST['action'] ) && 'elementor' === $_REQUEST['action'] && is_admin() ) {
                add_action( 'init', array( $this, 'register_wc_hooks_for_elementor' ), 5 );
            }
            ...
            add_action('woocommerce_after_shop_loop_item_title', 'woocommerce_template_loop_rating', 17);


            add_action('skudmart/action/add_count_up_timer_in_product_listing', array( $this, 'add_count_up_timer_in_product_listing' ), 1 );

            add_action('skudmart/action/shop_loop_item_action_top', function(){ echo '<div class="wrap-addto">'; }, 5 );
            add_action('skudmart/action/shop_loop_item_action_top', array( $this, 'add_cart_btn' ), 10 );
            add_action('skudmart/action/shop_loop_item_action_top', array( $this, 'add_quick_view_btn' ), 20 );
            add_action('skudmart/action/shop_loop_item_action_top', array( $this, 'add_compare_btn' ), 30 );
            add_action('skudmart/action/shop_loop_item_action_top', array( $this, 'add_wishlist_btn' ), 40 );
            add_action('skudmart/action/shop_loop_item_action_top', function(){ echo '</div>'; }, 50 );

            add_action('skudmart/action/shop_loop_item_action', function(){ echo '<div class="wrap-addto">'; }, 5 );
            add_action('skudmart/action/shop_loop_item_action', array( $this, 'add_cart_btn' ), 10 );
            add_action('skudmart/action/shop_loop_item_action', array( $this, 'add_quick_view_btn' ), 20 );
            add_action('skudmart/action/shop_loop_item_action', array( $this, 'add_compare_btn' ), 30 );
            add_action('skudmart/action/shop_loop_item_action', array( $this, 'add_wishlist_btn' ), 40 );
            add_action('skudmart/action/shop_loop_item_action', function(){ echo '</div>'; }, 50 );
            ...
            add_action('skudmart/action/after_primary', array( $this, 'move_after_product_summary_to_bottom') );
        ...
        public function hook_for_after_init(){
             ...
             */
            if( skudmart_string_to_bool( skudmart_get_option('product_single_hide_product_title', 'no') ) ){
                remove_action('woocommerce_single_product_summary', 'woocommerce_template_single_title', 5);
            }
            /**
             * Moving tabs to summary
             */
            if( !skudmart_string_to_bool( skudmart_get_option('move_woo_tabs_to_bottom', 'no')) && empty($_GET['product_quickview'])){
                add_action('woocommerce_single_product_summary', 'woocommerce_output_product_data_tabs', 55);
            }
            /**
             * Disable cross sell
             */
            if( skudmart_string_to_bool(skudmart_get_option('crosssell_products', 'off')) ){
        ...
        public function register_sidebars(){
            ...
            register_sidebar( array(
                'name'			=> esc_html_x( 'Shop Sidebar Filter', 'admin-view',  'skudmart' ),
        ...
        public function modify_product_thumbnail_size($size){
            $image_size = skudmart_get_wc_loop_prop('image_size');
            if(!empty($image_size)) {
                return $image_size;
            }
            return $size;
        ...
        public function theme_js(){
            ...
            wp_register_script('skudmart-product-gallery', get_theme_file_uri( '/assets/js/lib/lastudio-product-gallery'. $ext .'.js' ), array('jquery'), $theme_version, true);
            wp_register_script('skudmart-product-swatches', get_theme_file_uri( '/assets/js/lib/lastudio-swatches'. $ext .'.js' ), array('jquery'), $theme_version, true);
        }
        ...
        public function set_sidebar_for_shop( $sidebar ) {
            ...
            if( is_woocommerce() ){
                ...
                if( is_archive() ){
                    ...
                    if(skudmart_get_option('shop_global_sidebar', false)){
                        /*
                    ...
                    if( is_shop() ){
                        if( ($single_sidebar = skudmart_get_post_meta( wc_get_page_id( 'shop' ), 'sidebar')) && !empty($single_sidebar) ){
                            $sidebar = $single_sidebar;
                        }
                    }
                    if( is_product_taxonomy() ){
                        if( ($tax_sidebar = skudmart_get_post_meta( get_queried_object_id(), 'sidebar')) && !empty($tax_sidebar) ){
                            $sidebar = $tax_sidebar;
                        }
                    }
                }
                ...
                elseif( is_product() ){
                    $sidebar = skudmart_get_option('products_sidebar', $sidebar);

                    if(skudmart_get_option('products_global_sidebar', false)){
                        /*
                         * Return global sidebar if option will be enable
                         * We don't need more checking in context
                         */
                    ...
                    if( ($single_sidebar = skudmart_get_post_meta( get_the_ID(), 'sidebar')) && !empty($single_sidebar) ){
                        $sidebar = $single_sidebar;
                    }
                }
        ...
        public function add_product_thumbnails_to_loop(){
            ...
            if( 'on' == skudmart_get_option('woocommerce_enable_crossfade_effect') ){
                $with_second_image = true;
            }
            $disable_second_image = skudmart_get_wc_loop_prop('disable_alt_image');
            ...
            echo skudmart_render_variable( $output );
        ...
        public function shop_loop_item_excerpt(){
            $is_main_loop = skudmart_get_wc_loop_prop('is_main_loop', false);
            $loop_layout = skudmart_get_wc_loop_prop('loop_layout', 'grid');
            $loop_style = skudmart_get_wc_loop_prop('loop_style', '1');
            ...
            if($product->is_on_sale()){
                ...
                if(!empty($sale_price_dates_to)){
                    ...
                    <div class="product_item--deals-info">
                        <p class="product-available"><?php echo esc_html__('Available', 'skudmart'); ?><span><?php echo esc_html($stock_available); ?></span></p>
                        <p class="product-sold"><?php echo esc_html__('Sold', 'skudmart'); ?><span><?php echo esc_html($stock_sold); ?></span></p>
                        <div class="progress">
        ...
        public function add_quick_view_btn(){
            if( 'on' == skudmart_get_option('woocommerce_show_quickview_btn', 'off') ){
                global $product;
                printf(
                    ...
                    esc_attr_x('Quick View', 'front-view', 'skudmart'),
                    esc_attr_x('Quick View', 'front-view', 'skudmart')
                );
            }
        }
        ...
        public function add_cart_btn(){
            if( skudmart_get_option('catalog_mode', 'off') != 'on' && skudmart_get_option('woocommerce_show_addcart_btn', 'on') == 'on' ) {
        ...
        public function add_compare_btn(){
            ...
            if( skudmart_get_option('woocommerce_show_compare_btn', 'off') == 'on' ) {
                if ( !empty($yith_woocompare->obj) ) {
                    ...
                    printf(
                        ...
                        esc_attr($css_class),
                        esc_url($url),
                        esc_attr_x('Compare','front-view', 'skudmart'),
                    ...
                    $url = '#';
                    $text = esc_html_x('Compare','front-view', 'skudmart');
                    printf(
                        '<a class="%s" href="%s" title="%s" rel="nofollow" data-product_title="%s" data-product_id="%s"><span class="labtn-icon labtn-icon-compare"></span><span class="labtn-text">%s</span></a>',
                        esc_attr($css_class),
                        esc_url($url),
        ...
        public function add_wishlist_btn(){
            ...
            if(skudmart_get_option('woocommerce_show_wishlist_btn', 'off') == 'on'){
                global $product;
                if (function_exists('YITH_WCWL')) {
                    ...
                    if (YITH_WCWL()->is_product_in_wishlist($product->get_id(), $default_wishlist)) {
                        $text = esc_html_x('View Wishlist', 'front-view', 'skudmart');
                        $class = 'add_wishlist la-yith-wishlist button added';
                        $url = YITH_WCWL()->get_wishlist_url('');
                    }
                    else {
                        $text = esc_html_x('Add to Wishlist', 'front-view', 'skudmart');
                ...
                elseif(class_exists('TInvWL_Public_AddToWishlist')){
                    ...
                    if(isset($user_wishlist[0], $user_wishlist[0]['in']) && $user_wishlist[0]['in']){
                        ...
                        $url = '#';
                        $text = esc_html_x('Wishlist', 'front-view', 'skudmart');
                    }
                    printf(
                        '<a class="%s" href="%s" title="%s" rel="nofollow" data-product_title="%s" data-product_id="%s"><span class="labtn-icon labtn-icon-wishlist"></span><span class="labtn-text">%s</span></a>',
                        esc_attr($class),
                    ...
                    if(Skudmart_WooCommerce_Wishlist::is_product_in_wishlist($product->get_id())){
                        $class = 'add_wishlist button la-core-wishlist added';
                        $url = skudmart_get_wishlist_url();
                        $text = esc_html_x('View Wishlist', 'front-view', 'skudmart');
                    }
                    else{
        ...
        public function add_class_to_product_loop( $classes, $product ) {
            ...
            $enable_rating = skudmart_get_option('woocommerce_show_rating_on_catalog', 'off');
            if(get_option( 'woocommerce_enable_review_rating' ) === 'no'){
                $enable_rating = 'off';
            }
            $classes[] = 'prod-rating-' . esc_attr(skudmart_get_option('woocommerce_show_rating_on_catalog', 'off'));

            if(skudmart_string_to_bool( $enable_rating )){
        ...
        public function set_cookie_default(){
            if (isset($_GET['per_page']) && $per_page = $_GET['per_page']) {
                add_filter('skudmart/filter/get_product_per_page', array( $this, 'get_parameter_per_page'));
            }
        }

        public function get_parameter_per_page($per_page) {
            if (isset($_GET['per_page']) && ($_per_page = $_GET['per_page'])) {
                $param_allow = skudmart_woo_get_product_per_page_array();
        ...
        public function woocommerce_share(){
            if(skudmart_get_option('product_sharing') == 'on'){
                ...
                echo sprintf( '<label>%s</label>', esc_html_x('Share this product', 'front-end', 'skudmart') );
                skudmart_social_sharing($post_link,$post_title,$image);
                echo '</div>';
            }
        }
        ...
        public function modify_ajax_cart_fragments( $fragments ){
            ...
            $text = '<span class="la-cart-text">'. esc_html_x('%s items','front-view', 'skudmart') .'</span>';
        ...
        public function add_custom_tabs( $tabs ){
            ...
            if(skudmart_string_to_bool(skudmart_get_option('woo_enable_custom_tab'))){
                $custom_tabs = skudmart_get_option('woo_custom_tabs');
        ...
        public function callback_custom_tab_content( $tab_key, $tab_instance ){
            if(!empty($tab_instance['custom_content'])){
                echo wp_kses_post( skudmart_transfer_text_to_format($tab_instance['custom_content'], true) );
            }
        }
...
new Skudmart_WooCommerce_Config();
<?php
/**
 * Skudmart Breadcrumb Trail class
 *
 * @package Skudmart WordPress theme
 * @since 1.0.0
 */

if ( !function_exists('skudmart_breadcrumb_trail') ) {
    function skudmart_breadcrumb_trail( $args = array() ) {
        // Return if breadcrumbs are disabled
        if ( ! skudmart_has_breadcrumbs() || is_front_page() ) {
            return;
        }
...
if( !function_exists('skudmart_sp_breadcrumbs_before') ) {
    function skudmart_sp_breadcrumbs_before() {
        $classes = 'site-breadcrumbs';
        echo '<div class="'. esc_attr($classes) .'">';
    }
}
add_action( 'seopress_breadcrumbs_before_html', 'skudmart_sp_breadcrumbs_before' );

if(!function_exists('skudmart_sp_breadcrumbs_after')){
    function skudmart_sp_breadcrumbs_after() {
        echo '</div>';
    }
}
add_action( 'seopress_breadcrumbs_after_html', 'skudmart_sp_breadcrumbs_after' );
...
if( !class_exists('Skudmart_Breadcrumb_Trail') ) {

    class Skudmart_Breadcrumb_Trail {
        ...
        /**
         * Sets up the breadcrumb trail properties.  Calls the `Skudmart_Breadcrumb_Trail::add_items()` method
         ...
         *     @type array     $labels         Text labels. @see Skudmart_Breadcrumb_Trail::set_labels()
         *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Skudmart_Breadcrumb_Trail::set_post_taxonomy()
        ...
        public function get_trail() {
            ...
            $breadcrumb    	= '';
            $separator      = apply_filters( 'breadcrumb_trail_separator', skudmart_get_option('breadcrumb_separator', '>') );
            ...
            if ( 0 < $item_count ) {
                ...
                $breadcrumb .= '<ul class="trail-items"'. skudmart_get_schema_markup( 'breadcrumb' ) .'>';
                ...
                foreach ( $this->items as $item ) {
                    ...
                    $item = !empty( $matches ) ? sprintf( '%s<span'. skudmart_get_schema_markup( 'author_name' ) .'>%s</span>%s', $matches[1], $matches[2], $matches[3] ) : sprintf( '<span'. skudmart_get_schema_markup( 'author_name' ) .'>%s</span>', $item );
                    ...
                    $attributes = 'class="' . $item_class . '"'. skudmart_get_schema_markup( 'breadcrumb_list' ) .'';

                    // Separator
                    if ( $item_count === $item_position ) {
                        $sep = '';
                        $current_url = home_url(add_query_arg(null, null));
                        $tmp = sprintf( '<meta content="%s"'. skudmart_get_schema_markup( 'item' ) .' />', $current_url );
                        $item .= $tmp;
                    } else {
                        $sep = $separator;
                    }
                    ...
                    $meta = sprintf( '<meta content="%s"'. skudmart_get_schema_markup( 'position' ) .' />', absint( $item_position ) );
                ...
                $breadcrumb = sprintf(
                    '<%1$s aria-label="%2$s" class="site-breadcrumbs'. $p_class .'"'. skudmart_get_schema_markup( 'breadcrumb_itemprop' ) .'>%3$s%4$s%5$s</%1$s>',
        ...
        public function trail() {
            ...
            echo skudmart_render_variable( $this->get_trail() );
        }
        ...
        protected function set_labels() {
            ...
            $defaults = array(
                'aria_label'          	=> esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'skudmart' ),
                'home'           		=> skudmart_get_option( 'breadcrumb_translation_home', esc_html__( 'Home', 'skudmart' ) ),
                'error_404'           	=> skudmart_get_option( 'breadcrumb_translation_error', esc_html__( '404 Not Found', 'skudmart' ) ),
                'archives'            	=> esc_html__( 'Archives',                              'skudmart' ),
                // Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
                'search'           		=> skudmart_get_option( 'breadcrumb_translation_search', esc_html__( 'Search results for', 'skudmart' ) ),
                // Translators: %s is the page number.
                'paged'               	=> esc_html__( 'Page %s',                               'skudmart' ),
                // Translators: Minute archive title. %s is the minute time format.
                'archive_minute'      	=> esc_html__( 'Minute %s',                             'skudmart' ),
                // Translators: Weekly archive title. %s is the week date format.
                'archive_week'        	=> esc_html__( 'Week %s',                               'skudmart' ),
        ...
        protected function add_network_home_link() {
            ...
            // Home item
            $setting = skudmart_get_option( 'breadcrumb_home_item', 'icon' );
            ...
            if ( is_multisite() && ! is_main_site() && true === $this->args['network'] )
                $this->items[] = sprintf( '<a href="%s" rel="home" aria-label="' .$this->labels['home']. '" itemprop="item">%s</a>', esc_url( network_home_url() ), $icon, $text );
        }
        ...
        protected function add_site_home_link() {
            ...
            $this->items[] = sprintf( '<a href="%s"%s aria-label="' .$this->labels['home']. '" itemprop="item">%s%s</a>', esc_url( home_url() ), $rel, $icon, $label );
        }
        ...
        protected function add_singular_items() {
            ...
            // Post type taxonomy
            $posts_tax 		= skudmart_get_option( 'breadcrumb_posts_taxonomy', 'category' );
            $products_tax 	= skudmart_get_option( 'breadcrumb_products_taxonomy', 'shop' );
            $portfolio_tax 	= skudmart_get_option( 'breadcrumb_portfolio_taxonomy', 'la_portfolio_category' );
            ...
            if ( 'post' === $post->post_type && 'none' != $posts_tax ) {
                ...
                if ( 'blog' == $posts_tax
                    ...
                    $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_permalink( $page_id ) ), get_the_title( $page_id ) );
                }
                else {
            ...
            // If the post type is 'product'.
                ...
                if ( 'shop' == $products_tax ) {
                    ...
                    $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_permalink( $shop_id ) ), get_the_title( $shop_id ) );
                } 
                else {
            ...
            // If the post type is 'lastudio_portfolio'.
                ...
                $portfolio_id = skudmart_get_option( 'portfolio_page' );

                if ( 'portfolio' == $portfolio_tax ) {
                    if( ! empty( $portfolio_id )){
                        $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_permalink( $portfolio_id ) ), get_the_title( $portfolio_id ) );
                    }
                    else{
                        $post_type_object = get_post_type_object( 'la_portfolio' );
                        $label = ! empty( $post_type_object->labels->archive_title ) ? $post_type_object->labels->archive_title : $post_type_object->labels->name;
                        $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_post_type_archive_link( 'la_portfolio' ) ), esc_html($label) );
                    }
                }
                else {
            ...
            if ( $post_title = single_post_title( '', false ) ) {
                ...
                if ( 1 < get_query_var( 'page' ) || is_paged() )
                    $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_permalink( $post_id ) ), $post_title );
                    ...
                    if ( isset( $matches ) ) {
                        ...
                        foreach ( $matches as $match ) {
                            ...
                            if ( !empty( $post_types ) ) {
                                ...
                                $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_post_type_archive_link( $post_type_object->name ) ), $label );
            ...
            if ( false === $done_post_type && 1 === count( $taxonomy->object_type ) && post_type_exists( $taxonomy->object_type[0] ) ) {
                ...
                if ( 'post' === $taxonomy->object_type[0] ) {
                    ...
                    if ( 'posts' !== get_option( 'show_on_front' ) && 0 < $post_id )
                        $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_permalink( $post_id ) ), get_the_title( $post_id ) );
                    ...
                    $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( $url ), $label );
                }
            }
        ...
        protected function add_post_type_archive_items() {
            ...
            if ( is_paged() ){
                $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_post_type_archive_link( $post_type_object->name ) ), post_type_archive_title( '', false ) );
            }
            elseif ( true === $this->args['show_title'] ){
        ...
        protected function add_user_archive_items() {
            ...
            if ( is_paged() )
                $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_author_posts_url( $user_id ) ), get_the_author_meta( 'display_name', $user_id ) );
        ...
        protected function add_minute_hour_archive_items() {
            ...
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'skudmart' ) ) );
        }
        ...
        protected function add_minute_archive_items() {
            ...
            // Add the minute item.
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'skudmart' ) ) );
        }
        ...
        protected function add_hour_archive_items() {
            ...
            // Add the hour item.
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'skudmart' ) ) );
        }
        ...
        protected function add_day_archive_items() {
            ...
            $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'skudmart' ) ) );
            $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'skudmart' ) ) );
            $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'skudmart' ) ) );

            // Add the year and month items.
            $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_year_link( get_the_time( 'Y' ) ) ), $year );
            $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) ), $month );

            // Add the day item.
            if ( is_paged() )
                $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_day_link( get_the_time( 'Y' ) ), get_the_time( 'm' ), get_the_time( 'd' ) ), $day );
        ...
        protected function add_week_archive_items() {
            ...
            $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'skudmart' ) ) );
            $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'skudmart' ) ) );
        ...
        protected function add_search_items() {
            ...
            if ( is_paged() )
                $this->items[] = sprintf( '<a href="%s" itemprop="item">%s%s</a>', esc_url( get_search_link() ), '<span class="breadcrumb-search">'. $this->labels['search'] .'</span>', ' &#8220;'. get_search_query() .'&#8221;' );
        ...
        protected function add_post_parents( $post_id ) {
            ...
            while ( $post_id ) {
                ...
                $parents[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_permalink( $post_id ) ), get_the_title( $post_id ) );
        ...
        protected function add_post_hierarchy( $post_id ) {
            ...
            if ( $post_type_object->has_archive ) {
                ...
                $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_post_type_archive_link( $post_type ) ), $label );
            }
        }
        ...
        protected function add_post_terms( $post_id, $taxonomy ) {
            ...
            if ( $terms && ! is_wp_error( $terms ) ) {
                ...
                $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_term_link( $term, $taxonomy ) ), $term->name );
            }
        }
        ...
        function add_term_parents( $term_id, $taxonomy ) {
            ...
            while ( $term_id ) {
                ...
                $parents[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_term_link( $term, $taxonomy ) ), $term->name );
        ...
        protected function map_rewrite_tags( $post_id, $path ) {
            ...
            if ( is_array( $matches ) ) {
                ...
                foreach ( $matches as $match ) {
                    ...
                    if ( '%year%' == $tag )
                        $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'skudmart' ) ) ) );

                    // If using the %monthnum% tag, add a link to the monthly archive.
                    elseif ( '%monthnum%' == $tag )
                        $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'skudmart' ) ) ) );

                    // If using the %day% tag, add a link to the daily archive.
                    elseif ( '%day%' == $tag )
                        $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'skudmart' ) ) ) );

                    // If using the %author% tag, add a link to the post author archive.
                    elseif ( '%author%' == $tag )
                        $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_author_posts_url( $post->post_author ) ), get_the_author_meta( 'display_name', $post->post_author ) );

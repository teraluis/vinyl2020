...
class WC_Gateway_Amazon_Payments_Advanced extends WC_Payment_Gateway {
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'register_now' => array(
				...
				'type'        => 'title',
				'description' => __( 'If not <a class="register_now button-primary" href="#">CONFIGURE/REGISTER NOW</a>', 'woocommerce-gateway-amazon-payments-advanced' ),
			),
			'enabled' => array(
	...
	protected function process_async_auth( $order, $amazon_reference_id) {
		...
		$this->process_payment_with_async_authorize( $order, $amazon_reference_id );
	}
	...
	protected function process_payment_check_declined_error( $order_id, $result ) {
		...
		if ( in_array( $code, array( 'TransactionTimedOut' ) ) ) {
        ...
        if ( in_array( $code, array( 'AmazonRejected', 'ProcessingFailure' ) ) ) {
	...
	public function store_order_address_details( $order, $order_reference_details ) {
		...
		if ( isset( $order_reference_details->BillingAddress->PhysicalAddress ) ) {
			...
			$name = explode( ' ', ! empty( $buyer->Name ) ? (string) $buyer->Name : '' );

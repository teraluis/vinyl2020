( function( $ ) {
    ...
    $(window).on('load', function () {
        ...
        var _html = '<div class="elementor-panel-footer-sub-menu-item" data-device-mode="laptop"><i class="elementor-icon eicon-device-laptop" aria-hidden="true"></i><span class="elementor-title">'+ LaCustomBPFE.laptop.name +'</span><span class="elementor-description">'+LaCustomBPFE.laptop.text+'</span></div>';
    ...
    var LaStudioElementEditor = {
        ...
        init: function() {
            elementor.channels.editor.on( 'section:activated', LaStudioElementEditor.onAnimatedBoxSectionActivated );

            window.elementor.on( 'preview:loaded', function() {
                elementor.$preview[0].contentWindow.LaStudioElementEditor = LaStudioElementEditor;

                LaStudioElementEditor.onPreviewLoaded();
            });

            if(typeof window.elementorPro === "undefined"){
                elementor.hooks.addFilter('editor/style/styleText', LaStudioElementEditor.addCustomCss);
                elementor.settings.page.model.on('change', LaStudioElementEditor.addPageCustomCss);
                elementor.on('preview:loaded', LaStudioElementEditor.addPageCustomCss);
            }
        ...
        onAnimatedBoxSectionActivated: function( sectionName, editor ) {
            var editedElement = editor.getOption( 'editedElementView' ),
                prevEditedElement = window.LaStudioElementEditor.editedElement;
            ...
            if ( prevEditedElement && 'lastudio-animated-box' === prevEditedElement.model.get( 'widgetType' ) ) {
                ...
                window.LaStudioElementEditor.editedElement = null;
            }
            ...
            window.LaStudioElementEditor.editedElement = editedElement;
            window.LaStudioElementEditor.activeSection = sectionName;
    ...
    $( window ).on( 'elementor:init', LaStudioElementEditor.init );

    window.LaStudioElementEditor = LaStudioElementEditor;

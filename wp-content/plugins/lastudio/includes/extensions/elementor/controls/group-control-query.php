...
class Group_Control_Query extends Group_Control_Base {
	...
	protected function init_fields_by_name( $name ) {
		...
		$fields['post_type'] = [
			'label' => __( 'Source', 'lastudio-elements' ),
			'type' => Controls_Manager::SELECT,
			'options' => [
				'by_id' => __( 'Manual Selection', 'lastudio-elements' ),
		...
		$fields['query_include'] = [
			...
			'label' => __( 'Include', 'lastudio-elements' ),
		...
		$fields['posts_ids'] = [
			'label' => __( 'Search & Select', 'lastudio-elements' ),
			'type' => 'lastudio_query',
			'post_type' => '',
			'options' => [],
			'label_block' => true,
		...
		$fields['include'] = [
			'label' => __( 'Include By', 'lastudio-elements' ),
			'type' => Controls_Manager::SELECT2,
			'multiple' => true,
			'options' => [
				'terms' => __( 'Term', 'lastudio-elements' ),
				'authors' => __( 'Author', 'lastudio-elements' ),
			],
			'condition' => [
				'post_type!' => [
					'by_id',
		...
		$fields['include_term_ids'] = [
			'label' => __( 'Term', 'lastudio-elements' ),
			'type' => 'lastudio_query',
			'post_type' => '',
			'options' => [],
			'label_block' => true,
		...
		$fields['include_authors'] = [
			'label' => __( 'Author', 'lastudio-elements' ),
			'label_block' => true,
			'type' => 'lastudio_query',
			'multiple' => true,
			'default' => [],
		...
		$fields['query_exclude'] = [
			'type' => Controls_Manager::TAB,
			'label' => __( 'Exclude', 'lastudio-elements' ),
			'tabs_wrapper' => $tabs_wrapper,
			'condition' => [
				'post_type!' => [
					'by_id',
		...
		$fields['exclude'] = [
			'label' => __( 'Exclude By', 'lastudio-elements' ),
			'type' => Controls_Manager::SELECT2,
			'multiple' => true,
			'options' => [
				'current_post' => __( 'Current Post', 'lastudio-elements' ),
				'manual_selection' => __( 'Manual Selection', 'lastudio-elements' ),
				'terms' => __( 'Term', 'lastudio-elements' ),
				'authors' => __( 'Author', 'lastudio-elements' ),
			],
			'condition' => [
		...
		$fields['avoid_duplicates'] = [
			'label' => __( 'Avoid Duplicates', 'lastudio-elements' ),
			'type' => Controls_Manager::SWITCHER,
			'default' => '',
			'description' => __( 'Set to Yes to avoid duplicate posts from showing up. This only effects the frontend.', 'lastudio-elements' ),
			'tabs_wrapper' => $tabs_wrapper,
			'inner_tab' => $exclude_wrapper,
			'condition' => [
				'post_type!' => [
		...
		$fields['offset'] = [
			...
			'description' => __( 'Use this setting to skip over posts (e.g. \'2\' to skip over 2 posts).', 'lastudio-elements' ),
			'tabs_wrapper' => $tabs_wrapper,
			'inner_tab' => $exclude_wrapper,
		];

		$fields['select_date'] = [
			'label' => __( 'Date', 'lastudio-elements' ),
			'type' => Controls_Manager::SELECT,
			'post_type' => '',
			'options' => [
				'anytime' => __( 'All', 'lastudio-elements' ),
				'today' => __( 'Past Day', 'lastudio-elements' ),
				'week' => __( 'Past Week', 'lastudio-elements' ),
				'month'  => __( 'Past Month', 'lastudio-elements' ),
				'quarter' => __( 'Past Quarter', 'lastudio-elements' ),
				'year' => __( 'Past Year', 'lastudio-elements' ),
				'exact' => __( 'Custom', 'lastudio-elements' ),
			],
			'default' => 'anytime',
			'label_block' => false,
			'multiple' => false,
		...
		$fields['date_before'] = [
			'label' => __( 'Before', 'lastudio-elements' ),
			...
			'filter_type' => 'date',
			'include_type' => true,
			'placeholder' => __( 'Choose', 'lastudio-elements' ),
			'condition' => [
				'select_date' => 'exact',
				'post_type!' => [
					'by_id',
			...
			'description' => __( 'Setting a ‘Before’ date will show all the posts published until the chosen date (inclusive).', 'lastudio-elements' ),
		];

		$fields['date_after'] = [
			'label' => __( 'After', 'lastudio-elements' ),
			'type' => Controls_Manager::DATE_TIME,
			'post_type' => '',
			'label_block' => false,
			'multiple' => false,
			...
			'description' => __( 'Setting an ‘After’ date will show all the posts published since the chosen date (inclusive).', 'lastudio-elements' ),
		];
		...
		$fields['orderby'] = [
			...
			'default' => 'post_date',
			'options' => [
				'post_date' => __( 'Date', 'lastudio-elements' ),
				'post_title' => __( 'Title', 'lastudio-elements' ),
		...
		$fields['posts_per_page'] = [
			'label' => __( 'Posts Per Page', 'lastudio-elements' ),
			'type' => Controls_Manager::NUMBER,
			'default' => 3,
			'condition' => [
				'post_type!' => 'current_query',
		...
		$fields['ignore_sticky_posts'] = [
			'label' => __( 'Ignore Sticky Posts', 'lastudio-elements' ),
			'type' => Controls_Manager::SWITCHER,
			'default' => 'yes',
			'condition' => [
				'post_type' => 'post',
			],
			'description' => __( 'Sticky-posts ordering is visible on frontend only', 'lastudio-elements' ),
		];

		$fields['query_id'] = [
			'label' => __( 'Query ID', 'lastudio-elements' ),
			'type' => Controls_Manager::TEXT,
			'default' => '',
			'description' => __( 'Give your Query a custom unique id to allow server side filtering', 'lastudio-elements' ),
			'separator' => 'before',
		];
	...
	protected function prepare_fields( $fields ) {
		...
		$post_types = lastudio_elementor_get_public_post_types( $post_type_args );

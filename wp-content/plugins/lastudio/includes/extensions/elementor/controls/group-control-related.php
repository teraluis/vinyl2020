...
class Group_Control_Related extends Group_Control_Query {
	...
	protected function init_fields_by_name( $name ) {
		...
		$fields['post_type']['options']['related'] = __( 'Related', 'lastudio-elements' );
		...
		$related_fallback = [
			'label' => __( 'Fallback', 'lastudio-elements' ),
			'type' => Controls_Manager::SELECT,
			'options' => [
				'fallback_none' => __( 'None', 'lastudio-elements' ),
				'fallback_by_id' => __( 'Manual Selection', 'lastudio-elements' ),
				'fallback_recent' => __( 'Recent Posts', 'lastudio-elements' ),
			],
			'default' => 'fallback_none',
			'description' => __( 'Displayed if no relevant results are found. Manual selection display order is random', 'lastudio-elements' ),
			'condition' => [
				'post_type' => 'related',
			],
			'separator' => 'before',
	...
	protected function get_supported_taxonomies() {
		...
		$public_types = lastudio_elementor_get_public_post_types();

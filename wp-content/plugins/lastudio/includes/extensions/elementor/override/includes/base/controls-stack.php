...
abstract class Controls_Stack extends Base_Object {
     ...
     */
    const RESPONSIVE_LAPTOP = 'laptop';

    /**
     * Responsive 'tabletportrait' device name.
     */
    const RESPONSIVE_TABLET_PORTRAIT = 'tabletportrait';
	...
	final public function add_responsive_control( $id, array $args, $options = [] ) {
		...
		if($id == '_inline_size' && isset($args['device_args']) && isset($args['min_affected_device'])){

		    $args['device_args'][self::RESPONSIVE_LAPTOP] = [
                'max' => 100,
                'required' => false,
            ];

		    $args['device_args'][self::RESPONSIVE_TABLET_PORTRAIT] = [
                'max' => 100,
                'required' => false,
            ];

            $args['min_affected_device'] = [
                Controls_Stack::RESPONSIVE_DESKTOP => Controls_Stack::RESPONSIVE_LAPTOP,
                Controls_Stack::RESPONSIVE_LAPTOP => Controls_Stack::RESPONSIVE_LAPTOP,
            ];
        }
		...
		$devices = [
            ...
            self::RESPONSIVE_LAPTOP,
            self::RESPONSIVE_TABLET_PORTRAIT,
		];
     ...
     */
	final public function remove_group_control( $group_id, $group_name ){
        $group = Plugin::$instance->controls_manager->get_control_groups( $group_name );

        // Filter which controls to display
        $filtered_fields = $group->get_fields();

        $group_controls_prefix = $group_id . '_';
        ...
        foreach ( $filtered_fields as $field_id => $field_args ) {
            ...
            $id = $group_controls_prefix . $field_id;

            if ( ! empty( $field_args['responsive'] ) ) {
                $this->remove_responsive_control( $id );
            }
            else {
                $this->remove_control( $id );
            }
        }
    }
}

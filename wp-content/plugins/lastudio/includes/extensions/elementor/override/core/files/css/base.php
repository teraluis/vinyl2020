...
abstract class Base extends Base_File {
	...
	public function add_control_rules( array $control, array $controls_stack, callable $value_callback, array $placeholders, array $replacements ) {
		...
		foreach ( $control['selectors'] as $selector => $css_property ) {
			...
			if ( ! $query && ! empty( $control['responsive'] ) && is_array( $control['responsive'] ) ) {
	...
	protected function parse_content() {
        ...
        return apply_filters('lastudio-elements/custom-widget-css/parse_content', $this->stylesheet_obj->__toString());
	}
	...
	private function init_stylesheet() {
		...
		$this->stylesheet_obj
			->add_device( 'mobile', 0 )
            ->add_device( 'tabletportrait', $breakpoints['sm'] )
            ->add_device( 'tablet', $breakpoints['md'] )
			->add_device( 'laptop', $breakpoints['lg'] )
			->add_device( 'desktop', $breakpoints['xl'] );
	}

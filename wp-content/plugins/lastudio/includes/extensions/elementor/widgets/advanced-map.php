 ...
 * Advanced_Map Widget
 */
class Advanced_Map extends LA_Widget_Base {
    ...
    protected function get_widget_title() {
        return esc_html__( 'Advanced Map', 'lastudio' );
    }
    ...
    protected function _register_controls() {
        ...
        $this->start_controls_section(
            ...
            array(
                'label' => esc_html__( 'Map Settings', 'lastudio' ),
            )
        );

        $key = apply_filters('LaStudioElement/advanced-map/api', '');
        ...
        if ( ! $key ) {
            ...
            $this->add_control(
                ...
                array(
                    ...
                    'raw'  => sprintf(
                        esc_html__( 'Please set Google maps API key before using this widget. You can create own API key  %1$s. Paste created key on %2$s', 'lastudio' ),
                        '<a target="_blank" href="https://developers.google.com/maps/documentation/javascript/get-api-key">' . esc_html__( 'here', 'lastudio' ) . '</a>',
                        '<a target="_blank" href="'. admin_url('themes.php?page=theme_options#tab=48') .'">' . esc_html__( 'settings page', 'lastudio' ) . '</a>'
                    )
                )
            );
        }

        $default_address = esc_html__( 'PO Box 16122 Collins Street West Victoria 8007 Australia', 'lastudio' );

        $this->add_control(
            'map_center',
            array(
                'label'       => esc_html__( 'Map Center', 'lastudio' ),
        ...
        $this->add_control(
            'zoom',
            array(
                'label'      => esc_html__( 'Initial Zoom', 'lastudio' ),
        ...
        $this->add_control(
            'scrollwheel',
            array(
                'label'   => esc_html__( 'Scrollwheel Zoom', 'lastudio' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'false',
                'options' => array(
                    'true'  => esc_html__( 'Enabled', 'lastudio' ),
                    'false' => esc_html__( 'Disabled', 'lastudio' ),
                ),
            )
        );
        ...
        $this->add_control(
            'zoom_controls',
            array(
                'label'   => esc_html__( 'Zoom Controls', 'lastudio' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'true',
                'options' => array(
                    'true'  => esc_html__( 'Show', 'lastudio' ),
                    'false' => esc_html__( 'Hide', 'lastudio' ),
                ),
            )
        );
        ...
        $this->add_control(
            'fullscreen_control',
            array(
                'label'   => esc_html__( 'Fullscreen Control', 'lastudio' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'true',
                'options' => array(
                    'true'  => esc_html__( 'Show', 'lastudio' ),
        ...
        $this->add_control(
            'street_view',
            array(
                'label'   => esc_html__( 'Street View Controls', 'lastudio' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'true',
                'options' => array(
                    'true'  => esc_html__( 'Show', 'lastudio' ),
        ...
        $this->add_control(
            'map_type',
            array(
                'label'   => esc_html__( 'Map Type Controls (Map/Satellite)', 'lastudio' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'true',
                'options' => array(
                    'true'  => esc_html__( 'Show', 'lastudio' ),
        ...
        $this->add_control(
            'drggable',
            array(
                'label'   => esc_html__( 'Is Map Draggable?', 'lastudio' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'true',
                'options' => array(
        ...
        $this->start_controls_section(
            'section_map_style',
            array(
                'label' => esc_html__( 'Map Style', 'lastudio' ),
            )
        );
        ...
        $this->add_responsive_control(
            'map_height',
            array(
                'label'       => esc_html__( 'Map Height', 'lastudio' ),
        ...
        $this->add_control(
            ...
            array(
                ...
                'label_block' => true,
                'description' => esc_html__( 'You can add own map styles within your theme. Add file with styles array in .json format into lastudio-elements/google-map-styles/ folder in your theme. File must be minified', 'lastudio' )
            )
        );
        ...
        $this->start_controls_section(
            'section_map_pins',
            array(
                'label' => esc_html__( 'Pins', 'lastudio' ),
            )
        );
        ...
        $repeater->add_control(
            'pin_address',
            array(
                'label'       => esc_html__( 'Pin Address', 'lastudio' ),
                'type'        => Controls_Manager::TEXT,
                'default'     => $default_address,
                'label_block' => true,
                'dynamic'     => array( 'active' => true ),
        ...
        $repeater->add_control(
            'pin_desc',
            array(
                'label'   => esc_html__( 'Pin Description', 'lastudio' ),
        ...
        $repeater->add_control(
            'pin_image',
            array(
                'label'   => esc_html__( 'Pin Icon', 'lastudio' ),
        ...
        $repeater->add_control(
            'pin_state',
            array(
                'label'   => esc_html__( 'Initial State', 'lastudio' ),
                'type'    => Controls_Manager::SELECT,
                'default' => 'visible',
                'options' => array(
                    'visible' => esc_html__( 'Vsible', 'lastudio' ),
                    'hidden'  => esc_html__( 'Hidden', 'lastudio' ),
                ),
            )
        );
    ...
    public function _get_available_map_styles() {
        ...
        $key           = md5( 'lastudio-elements-' . LASTUDIO_VERSION );
        $plugin_styles = get_transient( $key );

        if ( ! $plugin_styles ) {

            $plugin_styles = $this->_get_map_styles_from_path(
                LASTUDIO_PLUGIN_PATH . 'public/element/js/lib/google-maps/'
            );
        ...
        $parent_styles = $this->_get_map_styles_from_path(
            get_template_directory() . '/' . lastudio_elementor_get_template() . 'google-map-styles/'
        );

        if ( get_stylesheet_directory() !== get_template_directory() ) {
            $child_styles = $this->_get_map_styles_from_path(
                get_stylesheet_directory() . '/' . lastudio_elementor_get_template() . 'google-map-styles/'
            );
        } else {
            $child_styles = array();
        }
        ...
        return array_merge(
            array( 'default' => esc_html__( 'Default', 'lastudio' ) ),
            $plugin_styles,
            $parent_styles,
            $child_styles
        );
    ...
    private function get_location_coord( $location ) {
        ...
        $api_key = apply_filters('LaStudioElement/advanced-map/api', '');

        // Do nothing if api key not provided
        if ( ! $api_key ) {
            $message = esc_html__( 'Please set Google maps API key before using this widget.', 'lastudio' );
        ...
        if ( ! $coord ) {
            ...
            $message = esc_html__( 'Coordinates of this location not found', 'lastudio' );
    ...
    protected function render() {
        ...
        $init = apply_filters( 'LaStudioElement/addons/advanced-map/data-args', array(
            'center'            => $coordinates,

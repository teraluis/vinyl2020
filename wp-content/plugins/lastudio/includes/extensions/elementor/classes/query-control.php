<?php
namespace LaStudio_Element\Classes;
...
use LaStudio_Element\Controls\Query;
use LaStudio_Element\Controls\Group_Control_Related;
...
class Query_Control {
    ...
    protected function add_actions() {
        ...
        $this->sys_messages = apply_filters( 'LaStudioElement/popups_sys_messages', array(
            'invalid_mail'                => esc_html__( 'Please, provide valid mail', 'lastudio' ),
            'mailchimp'                   => esc_html__( 'Please, set up MailChimp API key and List ID', 'lastudio' ),
            'internal'                    => esc_html__( 'Internal error. Please, try again later', 'lastudio' ),
            'server_error'                => esc_html__( 'Server error. Please, try again later', 'lastudio' ),
            'subscribe_success'           => esc_html__( 'Success', 'lastudio' ),
        ) );

        add_action( 'wp_ajax_lastudio_elementor_subscribe_form_ajax', [ $this, 'subscribe_form_ajax' ] );
        add_action( 'wp_ajax_nopriv_lastudio_elementor_subscribe_form_ajax', [ $this, 'subscribe_form_ajax' ] );
    ...
    public function enqueue_frontend_localize_script(){
        wp_localize_script(
            ...
            'lastudio_elementor_subscribe_form_ajax',
            array(
    ...
    public function subscribe_form_ajax() {
        ...
        if ( ! wp_verify_nonce( isset($_POST['nonce']) ? $_POST['nonce'] : false, 'lastudio_elementor_subscribe_form_ajax' ) ) {
            $response = array(
        ...
        $api_key = apply_filters('LaStudioElement/mailchimp/api', '');
        $list_id = apply_filters('LaStudioElement/mailchimp/list_id', '');
        $double_opt = apply_filters('LaStudioElement/mailchimp/double_opt_in', '');

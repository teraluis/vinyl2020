...
class Post_Query {
	...
	public function get_query() {
        ...
        Query_Control::add_to_avoid_list( wp_list_pluck( $query->posts, 'ID' ) );
	...
	protected function set_common_args() {
		...
		$post_type = $this->get_widget_settings( 'post_type' );
		if ( 'by_id' === $post_type ) {
			$post_types = lastudio_elementor_get_public_post_types();
	...
	protected function set_avoid_duplicates() {
		if ( 'yes' === $this->get_widget_settings( 'avoid_duplicates' ) ) {
			...
			$post__not_in = array_merge( $post__not_in, Query_Control::$displayed_ids );
			$this->set_query_arg( 'post__not_in', $post__not_in );
		}
	}
	...
	public function pre_get_posts_query_filter( $wp_query ) {
		if ( $this->widget ) {
			 ...
			 *
			 * The dynamic portions of the hook name, `$query_id`, refers to the Widget name and Query ID respectively.
			 *
			 * @since 1.0.0
			 *
			 * @param \WP_Query     $wp_query

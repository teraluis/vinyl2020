...
if (!class_exists('LASF')) {
    class LASF
    {
            ...
            if (!empty(self::$args['options'])) {
                foreach (self::$args['options'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        LASF_Options::instance($key, $params);
                        if (!empty($value['show_in_customizer'])) {
                            $customize_options_value = (is_array($value['show_in_customizer'])) ? $value['show_in_customizer'] : $value;
                            $customize_options_params = $params;
                            $customize_options_params['args'] = $customize_options_value;
                            LASF_Customize_Options::instance($key, $customize_options_params);
                        }
                    }
                }
            }
            ...
            if (!empty(self::$args['customize_options'])) {
                foreach (self::$args['customize_options'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        $params['sections'] = self::$args['sections'][$key];
                        self::$inited[$key] = true;
                        LASF_Customize_Options::instance($key, $params);
                    }
                }
            }
            // setup metaboxes
            ...
            if (!empty(self::$args['metaboxes'])) {
                foreach (self::$args['metaboxes'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        $params['sections'] = self::$args['sections'][$key];
                        self::$inited[$key] = true;
                        LASF_Metabox::instance($key, $params);
                    }
                }
            }
            ...
            if (!empty(self::$args['profile_options'])) {
                foreach (self::$args['profile_options'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        $params['sections'] = self::$args['sections'][$key];
                        self::$inited[$key] = true;
                        LASF_Profile_Options::instance($key, $params);
                    }
                }
            }
            // setup shortcoders
            ...
            if (!empty(self::$args['shortcoders'])) {
                foreach (self::$args['shortcoders'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        $params['sections'] = self::$args['sections'][$key];
                        self::$inited[$key] = true;
                        LASF_Shortcoder::instance($key, $params);
                    }
                }
                // Once editor setup for gutenberg and media buttons
                if (!empty(LASF::$shortcode_instances)) {
                    LASF_Shortcoder::once_editor_setup();
                }
            }
            ...
            if (!empty(self::$args['taxonomy_options'])) {
                foreach (self::$args['taxonomy_options'] as $key => $value) {
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        $params['sections'] = self::$args['sections'][$key];
                        self::$inited[$key] = true;
                        LASF_Taxonomy_Options::instance($key, $params);
                    }
                }
            }
            // create widgets
            if (!empty(self::$args['widgets']) && class_exists('WP_Widget_Factory')) {
                ...
                foreach (self::$args['widgets'] as $key => $value) {
                    if (!isset(self::$inited[$key])) {
                        self::$inited[$key] = true;
                        $wp_widget_factory->register(LASF_Widget::instance($key, $value));
                    }
                }
            }

            do_action('lasf_loaded');
        }
        ...
        public static function include_plugin_file($file, $load = true) {
            $path = '';
            $file = ltrim($file, '/');
            $override = apply_filters('lasf_override', 'lasf-override');
        ...
        public static function maybe_include_field($type = '') {
            if (!class_exists('LASF_Field_' . $type) && class_exists('LASF_Fields')) {
        ...
        public static function add_admin_enqueue_scripts() {
            ...
            wp_enqueue_style('lasf', LASF::include_plugin_url('assets/css/lasf' . $min . '.css'), array(), '1.0.0', 'all');
            // rtl styles
            if (is_rtl()) {
                wp_enqueue_style('lasf-rtl', LASF::include_plugin_url('assets/css/lasf-rtl' . $min . '.css'), array(), '1.0.0', 'all');
            }
            // framework core scripts
            wp_enqueue_script('lasf-plugins', LASF::include_plugin_url('assets/js/lasf-plugins' . $min . '.js'), array(), '1.0.0', true);
            wp_enqueue_script('lasf', LASF::include_plugin_url('assets/js/lasf' . $min . '.js'), array('lasf-plugins'), '1.0.0', true);
            wp_localize_script('lasf', 'lasf_vars', array(
                'color_palette' => apply_filters('lasf_color_palette', array()),
                'i18n' => array(
                    'confirm' => esc_html__('Are you sure?', 'lastudio'),
                    'reset_notification' => esc_html__('Restoring options.', 'lastudio'),
                    'import_notification' => esc_html__('Importing options.', 'lastudio'),
                ),
            ));
            ...
            if (!empty(self::$fields)) {
                foreach (self::$fields as $field) {
                    if (!empty($field['type'])) {
                        $classname = 'LASF_Field_' . $field['type'];
        ...
        public static function field($field = array(), $value = '', $unique = '', $where = '', $parent = '') {
            ...
            if (!empty($field['_notice'])) {
                ...
                $field = array();
                $field['content'] = sprintf(esc_html__('Ooops! This field type (%s) can not be used here, yet.', 'lastudio'), '<strong>' . $field_type . '</strong>');
                $field['type'] = 'notice';
                $field['style'] = 'danger';
            }
            $depend = '';
            ...
            $is_pseudo = (!empty($field['pseudo'])) ? ' lasf-pseudo-field' : '';
            ...
            if (!empty($field_type)) {
                echo '<div class="lasf-field lasf-field-' . $field_type . $is_pseudo . $class . $hidden . '"' . $depend . '>';
                if (!empty($field['title'])) {
                    $subtitle = (!empty($field['subtitle'])) ? '<p class="lasf-text-subtitle">' . $field['subtitle'] . '</p>' : '';
                    echo '<div class="lasf-title"><h4>' . $field['title'] . '</h4>' . $subtitle . '</div>';
                }
                echo (!empty($field['title'])) ? '<div class="lasf-fieldset">' : '';
                $value = (!isset($value) && isset($field['default'])) ? $field['default'] : $value;
                $value = (isset($field['value'])) ? $field['value'] : $value;
                self::maybe_include_field($field_type);
                $classname = 'LASF_Field_' . $field_type;
                if (class_exists($classname)) {
                    $instance = new $classname($field, $value, $unique, $where, $parent);
                    $instance->render();
                }
                else {
                    echo '<p>' . esc_html__('This field class is not available!', 'lastudio') . '</p>';
                }
            }
            else {
                echo '<p>' . esc_html__('This type is not found!', 'lastudio') . '</p>';
            }

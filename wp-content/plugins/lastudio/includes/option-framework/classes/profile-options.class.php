 ...
 *
 */
if( ! class_exists( 'LASF_Profile_Options' ) ) {
  class LASF_Profile_Options extends LASF_Abstract{
    ...
    public function render_profile_form_fields( $profileuser ) {
      ...
      $errors     = ( ! empty( $profile_id ) ) ? get_user_meta( $profile_id, '_lasf_errors', true ) : array();
      $errors     = ( ! empty( $errors ) ) ? $errors : array();

      // clear errors
      if( ! empty( $errors ) ) {
        delete_user_meta( $profile_id, '_lasf_errors' );
      }

      echo '<div class="lasf lasf-profile lasf-onload">';

      wp_nonce_field( 'lasf_profile_nonce', 'lasf_profile_nonce' );
      ...
      foreach( $this->sections as $section ) {
        ...
        if( ! empty( $section['fields'] ) ) {
          foreach( $section['fields'] as $field ) {
            ...
            LASF::field( $field, $this->get_meta_value( $profile_id, $field ), $this->unique, 'profile' );
    ...
    public function save_profile( $user_id ) {
      ...
      if ( wp_verify_nonce( lasf_get_var( 'lasf_profile_nonce' ), 'lasf_profile_nonce' ) ) {
        ...
        foreach ( $this->sections as $section ) {
          ...
          $request = apply_filters( "lasf_{$this->unique}_save", $request, $user_id, $this );

          do_action( "lasf_{$this->unique}_save_before", $request, $user_id, $this );
          ...
          if( empty( $request ) ) {
            ...
            if( ! empty( $errors ) ) {
              update_user_meta( $user_id, '_lasf_errors', $errors );
            }

          }

          do_action( "lasf_{$this->unique}_saved", $request, $user_id, $this );

          do_action( "lasf_{$this->unique}_save_after", $request, $user_id, $this );

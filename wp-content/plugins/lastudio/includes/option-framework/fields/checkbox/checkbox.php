 ...
 *
 */
if( ! class_exists( 'LASF_Field_checkbox' ) ) {
  class LASF_Field_checkbox extends LASF_Fields {
    ...
    public function render() {
      ...
      $inline_class = ( $args['inline'] ) ? ' class="lasf--inline-list"' : '';
      ...
      if( ! empty( $this->field['options'] ) ) {
        ...
        if( ! empty( $options ) ) {
          ...
          echo ( ! empty( $this->field['empty_message'] ) ) ? $this->field['empty_message'] : esc_html__( 'No data provided for this option type.', 'lastudio' );

        }

      } else {

        echo '<label class="lasf-checkbox">';
        echo '<input type="hidden" name="'. $this->field_name() .'" value="'. $this->value .'" class="lasf--input"'. $this->field_attributes() .'/>';
        echo '<input type="checkbox" class="lasf--checkbox"'. checked( $this->value, 1, false ) .'/>';

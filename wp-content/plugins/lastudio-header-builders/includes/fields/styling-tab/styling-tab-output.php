 ...
 *
 * @since       1.0.0
 * @return      global variable ($GLOBALS['lahb-dynamic-styles'])
 */
function lahb_styling_tab_output( $atts = array(), $el = '', $class = '', $class_hover = '' ) {
	...
	if ( ! empty( $tablet_style ) ) :
		$tablet_styles .= '@media only screen and ( max-width:1279px ) {' . $class . ' { '  . $tablet_style  . '} }';
	endif;

	if ( ! empty( $tablet_style_hover ) ) :
		$tablet_styles .= '@media only screen and ( max-width:1279px ) {' . $class_hover . ':hover { ' . $tablet_style_hover . '} }';
	endif;
	...
	if ( ! empty( $mobile_style ) ) :
		$mobile_styles .= '@media ( max-width: 799px ) {' . $class . ' { ' . $mobile_style  . '} }';
	endif;

	if ( ! empty( $mobile_style_hover ) ) :
		$mobile_styles .= '@media ( max-width: 799px ) {' . $class_hover . ':hover { ' . $mobile_style_hover . '} }';
	endif;

	if(isset($atts['area_vertical'])){
//        la_log([
//            'case' => 'one',
//            'desc' => 'return desktop style',
//            '$el' => $el,
//            '$class' => $class
//        ]);
	    return $styles;
...
//	        la_log([
//                'case' => 'two',
//                'desc' => 'return tablet + mobile style',
//	            '$el' => $el,
//                '$class' => $class
//            ]);

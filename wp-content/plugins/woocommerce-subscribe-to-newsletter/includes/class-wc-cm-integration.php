...
class WC_CM_Integration {
	...
	public function get_lists() {
		if ( ! $cmonitor_lists = get_transient( 'wc_cm_list_' . md5( $this->api_key ) ) ) {
			...
			if ( $result->was_successful() ) {
				...
				echo '<div class="error"><p>' . esc_html__('Unable to load data from Campaign Monitor - check your API key.', 'wc_subscribe_to_newsletter') . '</p></div>';
			}
		}
	...
	public function show_stats() {
		...
		if ( ! $stats = get_transient( 'woocommerce_cmonitor_stats' ) ) {
			...
			if ( $result->was_successful() ) {
				...
				$stats .= '<li><strong style="font-size:3em;">' . esc_html( $result->response->TotalActiveSubscribers ) . '</strong> ' . esc_html__( 'Total subscribers', 'wc_subscribe_to_newsletter' ) . '</li>';
				$stats .= '<li><strong style="font-size:3em;">' . esc_html( $result->response->NewActiveSubscribersToday ) . '</strong> ' . esc_html__( 'Subscribers today', 'wc_subscribe_to_newsletter' ) . '</li>';
				$stats .= '<li><strong style="font-size:3em;">' . esc_html( $result->response->NewActiveSubscribersThisMonth ) . '</strong> ' . esc_html__( 'Subscribers this month', 'wc_subscribe_to_newsletter' ) . '</li>';
				$stats .= '<li><strong style="font-size:3em;">' . esc_html( $result->response->UnsubscribesThisMonth ) . '</strong> ' . esc_html__( 'Unsubscribes this month', 'wc_subscribe_to_newsletter' ) . '</li>';
				$stats .= '</ul>';

				set_transient('woocommerce_cmonitor_stats', $stats, 60*60*1);

			} else {
				echo '<div class="error inline"><p>' . esc_html__('Unable to load stats from Campaign Monitor', 'wc_subscribe_to_newsletter') . '</p></div>';
			}
		}
	...
	public function subscribe( $first_name, $last_name, $email, $listid = 'false' ) {
		...
		if ( ! $succeed || ( is_callable( array( $result, 'was_successful' ) ) && ! $result->was_successful() ) ) {
			$message = __( 'Unexpected response from Campaign Monitor API.', 'wc_subscribe_to_newsletter' );
			if ( is_a( $result, 'CS_REST_Wrapper_Result ' ) ) {
				$message .= ' ' . sprintf( __( 'HTTP status code %s with following response: %s.', 'wc_subscribe_to_newsletter' ), esc_html( $result->http_status_code, print_r( $result->response, true ) ) );
			}

			// Email admin.
			wp_mail( get_option( 'admin_email' ), esc_html__( 'Email subscription failed (Campaign Monitor)', 'wc_subscribe_to_newsletter' ), $message );
		} else {
